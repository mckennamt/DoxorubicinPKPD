...
function cellData = predictCellTimecourse_only6(CellLine, funToFit, otherCellLines)

% function used to train model with only 6-hour exposure data to predict
% response to both 12-hour and 24-hour exposures

%% with left-out cell lines, optimize prediction parameters
%fitSettings = optimizeFittingParameters(otherCellLines, funToFit);
fitSettings = optimizeFittingParameters_only6(otherCellLines, funToFit);
%fitSettings = [5 3 .2];
...
%% train with 6 hour data to predict 12 and 24 hour data sets
...
ioi_testGroups = logical([0 1 0; 0 0 1]);
...
sfn = sprintf('%s_Predictions_IJCounts_only6.mat',CellLine);
save(sfn)
