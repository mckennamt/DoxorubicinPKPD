...
function cellData = predictCellTimecourse_only24(CellLine, funToFit, otherCellLines)

% function used to train model with only 24-hour exposure data to predict
% response to both 6-hour and 12-hour exposures

%% with left-out cell lines, optimize prediction parameters
%fitSettings = optimizeFittingParameters(otherCellLines, funToFit);
fitSettings = optimizeFittingParameters_only24(otherCellLines, funToFit);
%fitSettings = [5 3 .2];
...
%% train with 24 hour data to predict 6 and 12 hour data sets
...
% collect Test Data (6 hour, 12 hour)
ioi_testGroups = logical([1 0 0; 0 1 0]);
...
sfn = sprintf('%s_Predictions_IJCounts_only24.mat',CellLine);
save(sfn)
